...
// WrapCoordinate
// Wraps a coordinate onto the texture torus. Every step of the construction is toroidal: the texture is tiled across the screen, so a link that leaves one edge must re-enter at the other or the pairing stops being a bijection.
...
// Shortest signed distance from `from` to `to` on the torus. This is the step that makes the texture tileable: a link near an edge would otherwise record a delta nearly the width of the texture, when the same partner is a short hop away through the wrap.
...
{
  // Equation 3
  // Uses the form the paper gives as its own approximation: n = round(sigma^2 / 2).
  // The correction terms are dropped deliberately. Their placement is ambiguous in the PDF - the extracted layout admits both "sigma^2/2 + 1.46/sigma + ..." and "sigma^2 / (2 + 1.46/sigma + ...)" - so both were built and the achieved standard deviation measured against the requested one on a 254x254 texture:
  ...
  // For sigma >= 3 the first two agree exactly and both land on target; below that the additive reading diverges badly and is not even monotonic (sigma 0.8 asks for 6 shuffles and overshoots four-fold), while the divisor reading is only correct at 0.8.
  // Plain sigma^2/2 is the one form that tracks the target across the whole range, and it reproduces the footnote's claim that sigma = 0.8 is the floor produced by a single iteration.
...
ReSTIRPTPairingTexture GenerateReSTIRPTPairingTexture(uint32_t size, float sigma, uint32_t seed)
{
  ...
  // An odd edge length would leave one texel without a partner, since links are handed out in pairs.
  if(size < 2 || (size % 2) != 0)
  {
    return texture;
  }
  ...
  // Link indices
  // Each link index is planted twice. After shuffling, the two texels holding a given index are the pair, and the distance they have drifted apart is the delta.
  // Two independent random walks of standard deviation sigma/sqrt(2) give a separation of standard deviation sigma, which is why the shuffle count targets half the variance.
  ...
  // Random walk
  // Repeated 2x2 block shuffles move every index a small random step per iteration, which is what lands the separations on a Gaussian.
  ...
  {
    // Every other iteration slides the 2x2 grid diagonally by one, or block boundaries never move and an index stays confined to its original 2x2 cell forever.
    ...
    for(uint32_t blockY = 0; blockY < blockCount; ++blockY)
    {
      for(uint32_t blockX = 0; blockX < blockCount; ++blockX)
      {
        ...
        // A uniformly random permutation of the four cells. Shuffling the values rather than swapping a chosen pair keeps every iteration an exact permutation of the whole texture, so no index is ever lost or duplicated.
  ...
  // Pair collection
  // Locates the two texels carrying each link index. The paper resolves this on the GPU with a deliberate write race; on the CPU the pair is simply collected.
  ...
  for(uint32_t texel = 0; texel < texelCount; ++texel)
  {
    ...
    // Only reachable if the shuffle stopped being a permutation, which would invalidate every delta derived from it.
  ...
  // Deltas
  // Each pair records opposite deltas, so following either texel's delta lands on the other.
  ...
  for(uint32_t link = 0; link < linkCount; ++link)
  {
    ...
    // A delta beyond int8 range cannot be encoded. It is possible in principle for a very large sigma on a small texture; the texture is rejected rather than silently clamped, because a clamped delta would break reciprocity.
  ...
  // Involution check
  // Verifies the property the whole scheme rests on rather than assuming it: follow every texel's delta and require the partner to point back.
