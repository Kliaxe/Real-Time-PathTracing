...
#ifndef RESTIR_MATH_HLSLI
#define RESTIR_MATH_HLSLI

static const float RESTIR_PI = 3.1415926535;
...
float ComputeMFactor(float q0, float q1)
{
    return (q0 <= 0.0f)
        ? 1.0f
...
float ComputePairwiseMisWeight(float w0, float w1, float M0, float M1)
{
...
//
float CalculatePartialJacobian(const float Distance, const float3 RayDir, const float3 SampleNormal)
{
...
float CalculateJacobian(float3 recieverPos, float3 neighborReceiverPos, float3 neighborPosition, float3 neighborNormal)
{
	...
	CalculatePartialJacobian(recieverPos, neighborPosition, neighborNormal, NewDistanceSqr, NewCosine);
	CalculatePartialJacobian(neighborReceiverPos, neighborPosition, neighborNormal, OriginalDistanceSqr, OriginalCosine);
