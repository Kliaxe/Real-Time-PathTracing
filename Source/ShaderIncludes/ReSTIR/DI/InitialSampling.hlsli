...
#include "ReSTIR/Common/LightSampling/LocalLightSelection.hlsli"
...
DIInitialSamplingMisData ComputeInitialSamplingMisData(ReSTIRDIInitialSamplingParameters initialSamplingParams)
{
...
{
    return InitializeLocalLightSelectionContextUniform(localLightBufferRegion);
}
...
float2 SelectInfiniteLightUv(inout ReSTIRRandomSamplerState rng, DISurface surface, DILightInfo lightInfo)
{
    float2 uv;
    uv.x = GetNextRandom(rng);
    uv.y = GetNextRandom(rng);
...
void StreamInfiniteLightAtUvIntoReservoir(
    inout ReSTIRRandomSamplerState rng,
    DILightInfo lightInfo,
    DISurface surface,
    uint lightIndex,
    ...
    inout DILightSample o_selectedSample)
{
    ...
    float targetPdf = DIGetLightSampleTargetPdf(candidateSample, surface);
    float risRnd = GetNextRandom(rng);
    bool selected = StreamDIReservoirSample(state, lightIndex, uv, risRnd, targetPdf, invSourcePdf);
...
ReSTIRDIReservoir SampleInfiniteLights(
    inout ReSTIRRandomSamplerState rng,
    DISurface surface,
    uint numSamples,
    ...
    inout DILightSample o_selectedSample)
{
    ...
    for (uint i = 0; i < numSamples; i++)
    {
        ...
        SelectLightUniformly(rnd, infiniteLightBufferRegion, lightInfo, lightIndex, invSourcePdf);
        float2 uv = SelectInfiniteLightUv(rng, surface, lightInfo);
        StreamInfiniteLightAtUvIntoReservoir(rng, lightInfo, surface, lightIndex, uv, invSourcePdf, state, o_selectedSample);
    }
    ...
    out DILightSample o_lightSample)
{
    ...
    DILightSample infiniteSample = DIEmptyLightSample();  
    ReSTIRDIReservoir infiniteReservoir = SampleInfiniteLights(rng, surface,
    ...
    bool selectInfinite = CombineDIReservoirs(state, infiniteReservoir, GetNextRandom(rng), infiniteReservoir.targetPdf);
